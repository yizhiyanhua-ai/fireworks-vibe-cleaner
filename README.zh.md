...
以下实测早于 v0.2.0 的归档移除流程与扩展 Jev 选项，不能作为这些新能力的实测结果。
...
| 扩展选项后的 Jev 真实调用 | 1 次调用、20 个真实候选，全部返回 `review`；4,936 输入 token，2,620 ms |
| 新一轮真实原件清理 | 待具体清单人工确认，尚未执行；不把测试数据的删除量当成真实回收 |

[本轮 Jev 脱敏记录](docs/experiments/real-jev-v02.json)。没有准确率或优于规则的结论。
